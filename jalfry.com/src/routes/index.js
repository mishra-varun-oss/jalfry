	...
	origin: function (origin, callback) {
		...
		db.query(q, (err, results, fields) => {
			...
			if ((d.includes(h)) || (d.includes(rel))) {
				...
				callback(null, { origin: true });
			} else if (origin.includes("thehardbank.com")){
				callback(null, { origin: true });
			} else if (origin.includes("wrbl")) {
				callback(null, { origin: true })
			} else {
				console.log('no', h);
...
//	res.render(views_route + 'sso.hbs');
	res.render(views_route + 'sso_mpengs.hbs');
})
/*
router.get("/sso/mpengs", (req, res) => {
	res.render(views_route + 'sso_mpengs.hbs');
})
*/
...
router.post('/get_domain_info', (req, res) => {
	let app_name = req.body.app_name;
	let q = `SELECT * FROM domains WHERE domain = '${app_name}'`;
	db.query(q, (err, app_results) => {
		if (err) throw err;
		let q = `SELECT * FROM roles WHERE domain = '${app_name}'`;
		db.query(q, (err, role_results) => {
			if (err) throw err;

			let obj = {
				success: true,
				login_url: app_results[0].login_url,
				roles: role_results
			}
			res.send(obj);
		})
